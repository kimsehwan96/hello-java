...
* Action Lisnter 구현에는 몇가지 방법이 있는데..
* 제일 쉬운건 최종 컨테이너를 구현하는 프레임 안에 모든 요소 때려박고, 모든 멤버변수들을 접근하기 쉽게 하면 되는데
* 이쁘게 만들려고 하니 좀 더 고민해보자. 데이터를 처리하는 부분은 보이지 않는 요소로 Calculator 로직에서 모두 처리해보자

* 지금까지 확인 해 본 결과 필요한 메서드들

* 값을 입력받다가 +, -, / , *, 을 누르게 되면 
* 이전 값을 버퍼에 저장하고, 새로운 값을 또 입력받고.
* 이전에 입력받은 값과 현재 입력받은 값을 계산한 결과를 보여주고
* 만약 이전에 입력받은 값이 있다면, 연산자를 입력받고, 새로운 값을 입력받으면 처리하도록 ?
* 좀 구조를 아예 짜서 고민 해야겠다.